...
def configure_env_variables():  # for this usecase it makes sense to not make a separate shell script
    os.environ["UAVCAN__NODE__ID"] = "43"
    os.environ["UAVCAN__UDP__IFACE"] = "127.0.0.1"
    os.environ["UAVCAN__PUB__TEMPERATURE_SETPOINT__ID"] = "2345"
    os.environ["UAVCAN__PUB__TEMPERATURE_MEASUREMENT__ID"] = "2346"
    os.environ["UAVCAN__SUB__HEATER_VOLTAGE__ID"] = "2347"
    os.environ["UAVCAN__DIAGNOSTIC__SEVERITY"] = "2"
    # os.environ["UAVCAN__LOOPBACK"] = "1"


class ControllerNode:
    REGISTER_FILE = "ControllerNode.db"
    """
    ...
    def __init__(self) -> None:
        ...
        self._node = pyuavcan.application.make_node(node_info, ControllerNode.REGISTER_FILE)
        ...
        self._pub_temperature_set_point = self._node.make_publisher(uavcan.si.unit.temperature.Scalar_1_0,
                                                                    "temperature_setpoint")
        self._pub_temperature_measurement = self._node.make_publisher(uavcan.si.sample.temperature.Scalar_1_0,
                                                                      "temperature_measurement")
        self._sub_heater_voltage = self._node.make_subscriber(uavcan.si.voltage.Scalar_1_0, "heater_voltage")
    ...
    async def _serve_execute_command(
        ...
        if request.command == uavcan.node.ExecuteCommand_1_1.Request.COMMAND_FACTORY_RESET:
            try:
                os.unlink(ControllerNode.REGISTER_FILE)  # Reset to defaults by removing the register file.
