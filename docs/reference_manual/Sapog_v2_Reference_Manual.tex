...
\begin{ZubaxSimpleTable}{Quick definitions / abbreviations}{|l l | X|}\label{definitions_uavcan}
	...
	4 & Dynamic subject id. & A subject-id that does not have a fixed port number. The port number that will be used for communication needs to be the same on both communicating devices and this is achieved through a configuration procedure carried out by the configurator.
	
\end{ZubaxSimpleTable}

\begin{ZubaxSimpleTable}{Register name and its type}{|l | X|}\label{cli_software_recommendations}
	Name            & Register \\
	...
	uavcan.sub.setpoint.id & The integrator needs to set registers in the commander's register "uavcan.\textbf{pub}.setpoint.id" and in this ESC's register named "uavcan.\textbf{sub}.setpoint.id" to the same subject-id. Then the devices know that they are talking of the same setpoint message that a configurator has set a port for.\\
	uavcan.sub.readiness.id &  The integrator needs to set registers in the commander's register "uavcan.\textbf{pub}.readiness.id" and in this ESC's register named "uavcan.\textbf{sub}.readiness.id" to the same subject-id. Then the devices know that they are talking of the same readiness message that a configurator has set a port for.\\
	uavcan.pub.feedback.id & The node, which is going to be receiving the ESC feedback messages, needs to have the same register set with the same dynamic subject id number as the number that is going to be set in the registers of this ESC.\\
	uavcan.pub.status.id & The node, which is going to be receiving the ESC status messages, needs to have the same register set with the same dynamic subject id number as the number that is going to be set in the registers of this ESC.\\
	uavcan.pub.dynamics.id & The node, which is going to be receiving the ESC dynamics messages, needs to have the same register set with the same dynamic subject id number as the number that is going to be set in the registers of this ESC.\\
	...
	uavcan.pub.power.id &  \\
	ttl\_milliseconds & \\
	id\_in\_esc\_group & This index and the uavcan.sub.setpoint.id will determine the selected ESC for a controller that is trying to command ESCs on the CAN network.
\end{ZubaxSimpleTable}
...
\begin{ZubaxSimpleTable}{Register name and its type}{|l l | X|}\label{cli_software_recommendations}
