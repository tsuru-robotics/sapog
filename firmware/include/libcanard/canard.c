                                   ...
                                   const size_t presentation_layer_mtu)
{
        ...
        (tr->port_id <= CANARD_SUBJECT_ID_MAX))
    {
        ...
        ((tr->transfer_kind == CanardTransferKindRequest) || (tr->transfer_kind == CanardTransferKindResponse)) &&
...
void recursiveIterateCanardTreeNode(CanardTreeNode *canardTreeNode, CanardSubscriptionIteratorHandler *handler)
{
    (*handler)((CanardRxSubscription *) canardTreeNode);
    if (canardTreeNode->lr[0] != NULL)
    {
        recursiveIterateCanardTreeNode(canardTreeNode->lr[0], handler);
    }
    if (canardTreeNode->lr[1] != NULL)
    {
        recursiveIterateCanardTreeNode(canardTreeNode->lr[1], handler);
    }
}
...
void iterateOverRxSubscriptions(CanardInstance canardInstance, CanardSubscriptionIteratorHandler *handler)
{
    ...
    {
        recursiveIterateCanardTreeNode(canardInstance.rx_subscriptions[i], handler);
    }
}
